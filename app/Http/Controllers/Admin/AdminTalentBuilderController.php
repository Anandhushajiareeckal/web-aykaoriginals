...
class AdminTalentBuilderController extends Controller {
    public function index() {
        $sections = TalentSection::all()->keyBy('section_key');
        return view('admin.talent-builder.index', compact('sections'));
    }
    ...
    public function updateSection(Request $request, $key) {
        ...
        $section = TalentSection::firstOrCreate(['section_key' => $key]);

        if ($request->hasFile('video_file')) {
            $path = $request->file('video_file')->storePublicly('talent_builder/videos', 'public');
            ...
            $path = $request->file('image_file')->storePublicly('talent_builder/images', 'public');
